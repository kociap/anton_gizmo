...
namespace anton::gizmo {
    ...
    u32 generate_dial_3d_geometry(u32 const vertex_count, f32* const vertices_ptr, math::Vector3* const rotation_axes_ptr, f32* const scale_factors) {
        math::Vector3* const vertices = reinterpret_cast<math::Vector3*>(vertices_ptr);
        math::Vector3* const rotation_axes = reinterpret_cast<math::Vector3*>(rotation_axes_ptr);
    ...
    std::optional<f32> intersect_dial_3d(Ray, Dial_3D, f32 const* const, f32 const* const, u32 const, u32 const) {
        return std::nullopt;
    }
} // namespace anton::gizmo
