...
namespace anton {
    ...
    constexpr uint64_t popcount(uint64_t v) {
    ...
    //
    constexpr uint32_t clz(uint32_t v) {
        v |= (v >> 1);
        v |= (v >> 2);
        v |= (v >> 4);
        v |= (v >> 8);
    ...
    constexpr uint64_t clz(uint64_t v) {
        v |= (v >> 1);
        v |= (v >> 2);
        v |= (v >> 4);
        v |= (v >> 8);
    ...
    //
    constexpr uint32_t ilog2(uint32_t const v) {
        return 32 - clz(v);
    }

    constexpr uint64_t ilog2(uint64_t const v) {
        return 64 - clz(v);
    }
    ...
    //
    constexpr uint64_t ilog10(uint64_t const v) {
        constexpr uint64_t powers_of_10[] = {1ULL,
                                             10ULL,
                                             100ULL,
                                             1000ULL,
                                             10000ULL,
        ...
        uint64_t const temp = (ilog2(v) + 1) * 1233 >> 12;
        return temp - (v < powers_of_10[temp]);
    }

    // uint64_t ilog10(uint64_t const v) {
    //     if (v < 10)
    //         return 1;
    //     if (v < 100)
    //         return 2;
