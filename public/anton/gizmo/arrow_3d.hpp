...
namespace anton::gizmo {
    ...
    [[nodiscard]] math::Mat4 calculate_gizmo_transform(math::Mat4 const& world_transform, f32 fixed_scale_factor);

    // intersect_arrow_3d
    // Perform an intersection test of a ray against the bounding volumes of the arrow.
    // The arrow is located at (0, 0, 0) and is aligned with the -z axis before being transformed into world space.
    //
    // Parameters:
    ...
    // gizmo_transform - a transform to the world space generated by calculate_gizmo_transform.
    //
    // Returns:
    // Distance along ray's direction to the intersection point or null_optional if no intersection occured.
    //
    [[nodiscard]] Optional<f32> intersect_arrow_3d(math::Ray ray, Arrow_3D const& arrow, math::Mat4 const& gizmo_transform);
} // namespace anton::gizmo
